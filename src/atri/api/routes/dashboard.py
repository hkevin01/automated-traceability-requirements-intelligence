...
def _render_metric_cards(summary: dict[str, float | int]) -> str:
        ...
        for label, value in labels:
                cards.append(
                        f"<div class='card'><span>{escape(str(label))}</span><strong>{escape(str(value))}</strong></div>"
                )
        return "".join(cards)
...
def _render_bar_chart(title: str, series: dict[str, int], accent: str) -> str:
        ...
        for index, (label, value) in enumerate(series.items()):
                ...
                y = 18 + (index * 34)
                bars.append(
                        f"<g><text x='0' y='{y}'>{escape(label)}</text><rect x='170' y='{y - 12}' width='{width}' height='18' rx='9' fill='{accent}' /><text x='{180 + width}' y='{y}'>{value}</text></g>"
                )
...
def _render_dashboard_html(snapshot: dict[str, object]) -> str:
        ...
        coverage_width = int(float(coverage) * 100)
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        body {{ margin: 0; font-family: Inter, ui-sans-serif, system-ui, sans-serif; background: radial-gradient(circle at top, #fff7ed, var(--bg) 55%); color: var(--ink); }}
        ...
        .sub {{ max-width: 760px; color: var(--muted); font-size: 1rem; line-height: 1.5; }}
        .grid {{ display: grid; gap: 16px; }}
        .cards {{ grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); margin-bottom: 16px; }}
        .card, .panel {{ background: rgba(255, 250, 242, 0.92); border: 1px solid var(--line); border-radius: 20px; box-shadow: 0 18px 50px rgba(15, 23, 42, 0.08); }}
        ...
        .badge {{ padding: 10px 12px; border-radius: 999px; background: rgba(15, 118, 110, .09); color: var(--accent); font-weight: 600; font-size: .92rem; }}
        .section-title {{ margin: 28px 0 12px; font-size: 1.15rem; }}
        .coverage {{ display: grid; gap: 10px; }}
        .bar-shell {{ width: 100%; height: 16px; border-radius: 999px; background: rgba(15, 23, 42, 0.08); overflow: hidden; }}
        .bar-fill {{ width: {coverage_width}%; height: 100%; border-radius: inherit; background: linear-gradient(90deg, var(--accent), #22c55e); }}
...
<body>
    <main>
        <section class='hero'>
            ...
            <div class='sub'>A visual operating view of trace coverage, review activity, audit retention, and graph topology. The page is backed by the same persisted stores that power the API.</div>
        </section>
        ...
        <section class='panel coverage'>
            ...
            <div class='fine'>Coverage is shown as a percentage of trace completeness across the current summary snapshot.</div>
        </section>

        <h2 class='section-title'>Operational panels</h2>
        <section class='grid panels'>
            {_render_bar_chart('Review decisions', {{'Accepted': review_summary['accepted_reviews'], 'Rejected': review_summary['rejected_reviews'], 'Pending': review_summary['pending_reviews']}}, 'var(--accent)')}
            {_render_bar_chart('Audit events', audit_summary if audit_summary else {{'Total': 0}}, 'var(--accent-2)')}
            {_render_bar_chart('Graph topology', {{'Artifacts': graph_stats['artifact_count'], 'Links': graph_stats['link_count'], 'Roots': graph_stats['root_artifacts'], 'Leaves': graph_stats['leaf_artifacts']}}, 'var(--accent-3)')}
        </section>

        <section class='panel' style='margin-top:16px;'>
            <h3>Graph health</h3>
            <div class='badge-row'>
                <div class='badge'>Isolated artifacts: {graph_stats['isolated_artifacts']}</div>
                <div class='badge'>Average degree: {graph_stats['average_degree']}</div>
                <div class='badge'>Audit events: {audit_summary.get('total_events', 0)}</div>
                <div class='badge'>Accepted reviews: {review_summary['accepted_reviews']}</div>
            </div>
        </section>
    </main>
</body>
